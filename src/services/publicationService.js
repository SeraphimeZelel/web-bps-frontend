...
export const publicationService = {
    async addPublication(newPublication){
        try {
            const response = await apiClient.post('/publikasis',newPublication);
    ...
    async getPublications(){
        try {
            const response = await apiClient.get('/publikasis');
            return response.data;
        } catch (error){
    ...
    async editPublication(id, updatedPublication) {
        try {
            const response = await apiClient.put(`/publikasis/${id}`, updatedPublication);
            return response.data;
        } catch (error) {
    ...
    async deletePublication(id) {
        try {
            await apiClient.delete(`/publikasis/${id}`);
        } catch (error) {
